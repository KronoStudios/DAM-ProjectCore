...
class Game(SQLAlchemyBase, JSONModel):
    ...
    id = Column(BigInteger, primary_key=True)
    user1_id = Column(BigInteger, ForeignKey("users.id", onupdate="CASCADE", ondelete="CASCADE"), nullable=False)
    user2_id = Column(BigInteger, ForeignKey("users.id", onupdate="CASCADE", ondelete="CASCADE"), nullable=False)
    user_winner_id = Column(BigInteger, ForeignKey("users.id", onupdate="CASCADE", ondelete="CASCADE"), nullable=False)
    played_at = Column(DateTime, default=datetime.datetime.now, nullable=False)
    
    user1 = relationship("User", foreign_keys="Game.user1_id")
    user2 = relationship("User", foreign_keys="Game.user2_id")
    userWinner = relationship("User", foreign_keys="Game.user_winner_id")
